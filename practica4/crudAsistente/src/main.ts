...
(async ()=>{
  ...
  data.forEach((asistente: IAsistente) => {
    divTable += `<tr><td>${asistente.id}</td><td>${asistente.nombre}</td><td>${asistente.descripcion}</td><td>${asistente.precioBase}</td><td>${asistente.aforoMaximo}</td><td>${asistente.duracion}</td><td><button class="btn btn-delete">Eliminar</button></td><td><button class="btn btn-update">Actualizar</button> </td> </tr>`
  })
  divTable += `</table>`
