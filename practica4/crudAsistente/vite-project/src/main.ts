...
(async ()=>{
  ...
  const response = await fetch('http://localhost:5000/api/asistente')
  const data = await response.json()
  
  let divTable= `<table>`
  divTable += `<tr><th>Id</th><th>tipo</th><th>Modelo</th></tr>`
  data.forEach((asistente: IAsistente) => {
    divTable += `<tr><td>${asistente.id}</td><td>${asistente.tipo}</td><td>${asistente.modelo}</td><td><button class="btn btn-delete">Eliminar</button></td><td><button class="btn btn-update">Actualizar</button> </td> </tr>`
  })
  divTable += `</table>`
  ...
  document.querySelectorAll<HTMLButtonElement>('.btn-delete').forEach(btn=>{
    btn.addEventListener('click', async ()=>{ 
      ...
      console.log(id)
      await fetch(`http://localhost:5000/api/asistente/${id}`, {
        method: 'DELETE'
      })
      location.reload()
  ...
  document.querySelectorAll<HTMLButtonElement>('.btn-update').forEach(btn=>{
    btn.addEventListener('click', async ()=>{ 
      const id = btn.parentElement?.parentElement?.firstElementChild?.textContent
      const response = await fetch(`http://localhost:5000/api/asistente/${id}`)
      const data = await response.json()
